...
Request body format:
    ...
    `
    "operationName":"myOperationName", "query":"query myOperationName \{...\}"
    `
...
|**Data protection, backup and restore jobs**| | |
|createDataProtectionPlan|siteId, createInput (name, sourceSite, targetSite, description, active, priority, jobType, backupMechanism, retainedCopiesCount, autoAbort, autoAbortInMinutes, objects, targetRetentionCopiesCount, backupType)|Create a data protection plan for a site.|
|updateDataProtectionSettings|siteId, jobId, updateInput (name, sourceSite, targetSite, description, active, priority, jobType, backupMechanism, retainedCopiesCount, autoAbort, autoAbortInMinutes, objects, targetRetentionCopiesCount, backupType)|Update a data protection plan for a site.|
|deleteDataProtectionPlan|siteId, jobId|Delete a data protection plan.|
|createDataProtectionPlanSchedule|siteId, jobId, scheduleInput (scheduleExpression, runType)|Create a schedule for a data protection plan.|
|updateDataProtectionPlanSchedule|siteId, jobId, scheduleId, scheduleInput (scheduleExpression, runType)|Update a schedule for a data protection plan.|
|deleteDataProtectionPlanSchedule|siteId, jobId, scheduleId|Delete a schedule for a data protection plan.|
|previewDataProtectionPlanSchedule|siteId, jobId, scheduleInput (scheduleExpression, runType)|Preview a schedule for a data protection plan.|
|abortJob|siteId, jobId|Abort a running data protection plan.|
|instantJobRun|siteId, jobId, runType|Run a data protection plan immediately.|
|restoreData|siteId, executionId, jobId, settings (abortOnAccessRightsViolation, runAsCopyJob, restoreToDifferentDb, renameRestoredObjects, skipStatistics, skipJoinHashIndexes, targetDatabase, tableNameSuffix, objects)|Restore a retained copy to a site (same site or different target).|
|updateRestoreJobSettings|siteId, jobId, backupExecutionId, updateInput (abortOnAccessRightsViolation, runAsCopyJob, restoreToDifferentDb, renameRestoredObjects, skipStatistics, skipJoinHashIndexes, targetDatabase, tableNameSuffix, objects)|Update the settings for a restore job.|
|failOverData|siteId, executionId, jobId, failOverInput (name, sourceSite, targetSite, description, active, priority, jobType, backupMechanism, retainedCopiesCount, autoAbort, autoAbortInMinutes, objects, targetRetentionCopiesCount, backupType)|Create a failover job with failover information.|
|precheckRestore|siteId, executionId, jobId|Validate a snapshot for restore capability.|
...
Since a GraphQL call can span multiple services, error objects are returned as a list in the **Errors** portion of the response body. The structure of an error object is as described below:
    ...
    `
    "error": "message" \# message string "extensions": \{ \# additional metadata about the error "code" \# error code, e.g. UNAUTHORIZED, FORBIDDEN, BAD_USER_INPUT, etc. ... \# other error related details \}, \{ "path": [String] \# Query path(s) that resulted in this error "locations": ["line": Int, "column": Int] \# Query line(s) and column(s) that resulted in this error\}
    `
...
Trying to get the details of a site that does not exist.
    ...
    `
    \{ "errors"; [ \{ "message": "Site for the provided ID 'NOT_EXISTENT_SITE_ID' was not found", "locations": [ \{ "line": 2, "column": 3 \} ], "path": [ "stopSqlEngine" ], "extensions": \{ "id": "NOT_EXISTENT_SITE_ID", "status": "NOT_FOUND", "code": "BAD_USER_INPUT" \} \} ], "data"; \{ "stopSqlEngine"; null \}
    `
...
Calling the SQL engine **Start** operation on a site where that operation is currently not available (e.g. SQL engine is already running).
    ...
    `
    \{ "errors"; [ \{ "message": "SQL Engine START Operation Failed", "locations": [ \{ "line": 2, "column": 3 \} ], "path": [ "startSqlEngine" ], "extensions": \{ "status": "OPERATION_NOT_AVAILABLE", "availableOperations": [ "SCALE_UP_SCALE_DOWN", "SCALE_OUT_SCALE_IN", "STOP", "STORAGE_RESIZE" ], "details": "SQL Engine START Operation is not available at this time.", "code": "BAD_USER_INPUT" \} \} ], "data"; \{ "startSqlEngine"; null
    `
