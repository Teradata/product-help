...
Por ejemplo, cuando no hay autorización, si el usuario "Joe" inicia una consulta utilizando un enlace Teradata-to-Hive en un clúster no autorizado, Teradata automáticamente convierte el nombre de usuario "Joe" a mayúsculas y envía de forma predeterminada la consulta como el usuario "JOE" al sistema de destino. Los siguientes son los posibles resultados para este caso de uso cuando el conector de destino Hive no está asegurado:

|Escenario|Requisito|
|---------|------------|
|El usuario "JOE" no existe en el sistema de destino|Las consultas deben ejecutarse con un usuario existente en el sistema de destino como "hive". En este escenario, se requiere de asignación de usuario donde se asigna "JOE" en el sistema de Teradata al usuario "hive" en el sistema Hive.|
|El usuario no existe en el sistema de destino como "joe"|Se requiere de asignación de usuario donde se asigna "JOE" en el sistema de Teradata a "joe" en el sistema Hive de destino.|
|Existe en el destino como "JOE"|Este escenario no requiere una asignación de usuario|
...
|--------|------------|
|Usuario en el sistema de inicio (predeterminado)|El usuario del sistema de Teradata se pasa al sistema de destino como usuario suplantado con el nombre de usuario todo en minúsculas.|
