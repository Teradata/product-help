...
A private link service endpoint provides the connection between VantageCloud Lake and a trusted AWS or Azure account. Cross-platform connectivity is not supported. Use one of the following private link connections depending on your provider:

-   VantageCloud Lake on AWS connects through PrivateLink to an AWS account.


-   VantageCloud Lake on Azure connects through Private Link to an Azure account.
...
-   Select **Optional properties** to manually adjust connector properties. Use when implementing JWT SSO to the data source, see [Set up JWT SSO](esw1713987246219.md).
