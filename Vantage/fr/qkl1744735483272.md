...
-   **Exécuter en tant que copie**
    ...
    Sélectionnez cette option pour permettre la réexécution automatique de la tâche de restauration si la tâche de sauvegarde échoue. Lorsque cela se produit, généralement en raison d'objets supprimés sur le site cible, les objets sont restaurés sur une base de données interne avec un ID de base de données différent. Reportez-vous à [Exécuter en tant que copie](csm1744735504461.md).
