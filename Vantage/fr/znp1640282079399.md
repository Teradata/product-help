...
Affichez le [types d'autorisations](bbw1687364943833.md) avant de sélectionner une source de données. Veillez à vous connecter avec le DBA correct pour configurer un type d'autorisation.
...
-   [Facultatif] **Paramètres avancés** propose des paramètres de configuration supplémentaires pour la source de données, tels que l'ajout d'un pont ou une adaptation automatique de la charge.


-   Sélectionnez **Propriétés facultatives** pour ajuster manuellement les propriétés du connecteur. Utilisez cette option lors de l'implémentation de SSO pour JWT dans la source de données, reportez-vous à [Configurer SSO pour JWT](esw1713987246219.md).
...
**Remarque :** Pour décompresser le fichier d'archive TAR du module QueryGrid et obtenir le fichier RPM, utilisez la commande 
 ...
 `
   lors de l'enregistrement du nœud Teradata.
...
Pour configurer l'accès entre les systèmes, créez [serveurs étrangers](bbo1735955417476.md) entre les deux systèmes. Reportez-vous à [Configurer l'autorisation](bbw1687364943833.md).
